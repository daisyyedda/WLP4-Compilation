...
int main() {
  ...
  while(in >> s) {
    ...
    while (begin + end <= s.length()) {
      std::map<char, std::string> transition;
      transition[s[begin + end]] = curr;
      ...
      if (noRoute) {
        if (std::find(accept.begin(), accept.end(), curr) == accept.end()) {
          std::cerr << "ERROR: Token rejected." << std::endl;
          break;
        }
        if (s.substr(begin, end) != ".EMPTY") {
          std::cout << s.substr(begin, end) << std::endl;
          curr = start;
        }
        noRoute = false;
        begin += end;
